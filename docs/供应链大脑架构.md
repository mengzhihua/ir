...
- **成本分析 `CostService`**：按类型/仓库/承运商/趋势汇总，单均成本、目标 vs 实际。运费有 BMS 结算（`FREIGHT`/`TRANSPORT`）时不把 TMS 快照运费叠进看板和超标；没有 BMS 运费才回退 TMS。节省仍由成功动作 `expectedSaving` / 回写 `actualSaving` 累计（换商按运费×运价差，沙盘 apply 按条分摊）。
