...
- **闭环入口**：预警 `executeSuggested(alertId)` 与预测 `replenish/to-action` 均汇聚到 `ActionService`，把“发现问题/预测缺货 → 建议动作 → 跨系统执行 → 回写”串成一条自动化链路。补货建议和低库存的 `inTransit` 取自 SRM/SAP 未关闭 PO/ASN；采购建议会回写 `IR-PO-*`。
...
| `LOW_STOCK` | LOW_STOCK | 可用+在途 < 安全库存，键 `sku/warehouse` | MEDIUM | `SRM_PURCHASE_SUGGEST` |
