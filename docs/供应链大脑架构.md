...
- **成本分析 `CostService`**：按类型/仓库/承运商/趋势汇总，单均成本、目标 vs 实际。运费按订单对账：同一单有 BMS 结算（`FREIGHT`/`TRANSPORT`）时用账单，未入账运单仍用 TMS 快照。节省仍由成功动作 `expectedSaving` / 回写 `actualSaving` 累计（换商按运费×运价差，沙盘 apply 按条分摊）。
