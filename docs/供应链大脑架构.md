...
> 当前技术方案以 [供应链大脑构造.md](./供应链大脑构造.md) 和 README「对接」为准。本文保留分层、算法和数据模型说明；接入范围按代码写到十二套系统，不再把 SRM / SAP / BOM / INV / CRM / DMS / OA 当成未接入。
>
> 内容以 `ir` 仓库当前代码为准（后端 `com.ir.*`，端口 `8090`；前端 `5174`）。
...
| **BMS** 结算管理 | 计费结算 | 接收 WMS/TMS/OMS 单据 → AR/AP 自动计费 → 对账 → 发票 → 收付款 | 成本记录：`GET /api/open/cost/records` 与 `GET /api/open/ir/snapshots`。只读，不接受写指令 |

大脑与这四者的关系：**OMS→WMS→TMS→BMS** 是一笔订单从生成到履约再到结算的物理链路；IR 把这条链路“横切”出来，做全链路追踪与统一 KPI。

### 2.2 已接入的生态执行系统

这些系统各自记账。IR 拉 `GET /api/open/ir/snapshots`，并在有 API Key 时经 `/actions` 或专用口下发指令。没有 Key 时，只有 OMS / WMS / SRM / SAP / DMS 走登录后的业务口。

| 系统 | 定位 | 控制塔能做的事 |
| --- | --- | --- |
| **SRM** | PR→RFQ→PO→ASN→WMS→考核 | 采购建议、提交/审批 PR、催单 |
| **SAP** | MM/SD/FI/CO/PP | 库存与未清项快照；创建/释放 PR、释放生产订单 |
| **BOM** | EBOM/MBOM/SBOM、ECR/ECN | 展开 BOM，提交/审批/实施 ECN |
| **INV** | 进销项发票与税务 | 校验进项，提交/审核开票申请 |
| **DMS** | 经销商销售、维修、备件 | 按缺货生成补货单，按单号下发 |
| **CRM** | 线索、客户、商机、工单 | 推进商机阶段，升级工单 |
| **OA** | 组织、考勤、审批、薪资 | 发起审批、审批待办。登录模式不执行 |
...
- **客户端抽象**：`OmsClient / WmsClient / TmsClient / BmsClient / SrmClient`，以及 `HttpEcosystemClient` 覆盖 SAP / BOM / INV / CRM / DMS / OA。读快照、下指令、健康探测都从这里出去。BMS 只有读。
...
- **同步编排**：`SyncService`
  ...
  - 每个系统按类型拉取并落库：OMS→订单/库存/日销量；WMS→出库单/库存汇总；TMS→运单/运费；BMS→近 90 天成本；SRM/SAP/BOM/INV/CRM/DMS/OA→`ct_ext_snapshot`；
  - 每次写 `ct_sync_log`，并更新系统 `last_health_at / last_health_ok / last_error`；
  - `@Scheduled(cron = "${ir.sync.cron:0 0/5 * * * ?}")` 默认每 5 分钟自动同步一次。
- **接入配置**（`ct_system` 种子）：十一套执行系统默认 `MOCK` 且启用。切 HTTP 后的专用口和登录业务口见 README「对接」与 [供应链大脑构造.md](./供应链大脑构造.md)。
...
- **控制塔看板 `TowerService.overview()`**：
  - KPI：今日订单、在途订单/待处理、在途运输、延误运单、低库存 SKU、未处理预警、近 30 天总成本、单均成本、**OTIF**（准时足额交付率）、平均履约时长、目标达成分、NPS、待审批决策、延误 ASN；
  - 漏斗 `funnel`：OMS/WMS/TMS 各状态数量分布；
  - 成本趋势 `costTrend`（按天按成本类型）、仓库负载 `warehouseLoad`（WH-SH/BJ/GZ 待处理单量、库存量、低库存数）、TOP10 预警、系统健康；
  - 目标/平衡/供应摘要，以及 `command.nextActions` 统一队列（预警建议、PENDING 指令、待审批决策）；`POST /api/tower/command` 从总览直接下发。
- **全链路追踪 `TraceService`**：
...
- **成本分析 `CostService`**：按类型/仓库/承运商/趋势汇总，单均成本、目标 vs 实际。运费按订单对账：同一单有 BMS 结算（`FREIGHT`/`TRANSPORT`，含零额/负额）时用账单，未入账运单仍用 TMS 快照。来源标签按记录是否存在，不看净额符号。节省仍由成功动作 `expectedSaving` / 回写 `actualSaving` 累计（换商按运费×运价差，沙盘 apply 按条分摊）。

### ⑤ 行动层（Action）— `com.ir.action`

把决策“落下去”，形成闭环。

- **`ActionService.createAndExecute`**：根据动作类型前缀路由到目标系统（`OMS_/WMS_/TMS_` → 对应系统；`SAP_/BOM_/INV_/CRM_/DMS_/OA_` → 对应系统；其余 → `SRM`），构造 `ActionCommand` 调用对应客户端 `execute`，执行成功后 `mutateSnapshot` 同步回写本地快照（如改仓、挂起、加急优先级、取消、分配、换承运商、追轨迹刷新 ETA、调度），状态 `PENDING→RUNNING→SUCCESS/FAILED`。待办执行用条件更新抢占，避免并发双执行；复用待办时刷新 `expectedSaving`。
- **闭环入口**：预警 `executeSuggested(alertId)`、控制塔 `POST /api/tower/command`、预测 `replenish/to-action` 均汇聚到 `ActionService`，把“发现问题/预测缺货 → 建议动作 → 跨系统执行 → 回写”串成一条自动化链路。补货建议和低库存的 `inTransit` 按 SKU+仓库取自 SRM/SAP 未关闭 PO/ASN；采购建议回写 `IR-PO-{SYS}-{sku}-{warehouse}`。
...
- **补货建议 `replenish`**：对每个库存项跑 AUTO 预测，`日均 = 预测总量/horizon`，`安全库存 = 日均 × serviceDays`，`在途 = 同 SKU+仓库 未关闭 SRM/SAP PO 与 ASN 的较大值`（旧 PO 无仓号则按工厂映仓），`建议补货 = 预测总量 + 安全库存 − 可用 − 在途`，并推算 `stockoutDate`。可转 `SRM_PURCHASE_SUGGEST` 或 `WMS_REPLENISH`。
...
| `TMS_DELAY` | TMS_DELAY | 计划到达已过且非终态 | HIGH | `TMS_SYNC_TRACK` |
| `LOW_STOCK` | LOW_STOCK | 可用+本仓在途 < 安全库存，键 `sku/warehouse`；旧 SKU-only 键会迁过去并继续抑制 | MEDIUM | `SRM_PURCHASE_SUGGEST` |
...
预警状态：`OPEN → ACK / IGNORED / RESOLVED`。执行建议须一次处置里的全部动作都成功才 `RESOLVED` 并靠 `actionId` 抑制重开；批量里有失败则保持 OPEN，不打抑制，失败的补货/换商可以再点。条件消失后清抑制，下一轮才可新开。停用规则不自动关掉已有 OPEN。规则可在“规则”页增删改与启停。
...
| WMS | `WMS_ALLOCATE` / `WMS_REPLENISH` | 分配；同仓补货，源仓不同则跨仓调拨 |
| TMS | `TMS_DISPATCH` / `TMS_SYNC_TRACK` / `TMS_SWITCH_CARRIER` | 调度、追轨迹、切换承运商 |
| SRM | `SRM_PURCHASE_SUGGEST` / `SRM_SUBMIT_PR` / `SRM_APPROVE_PR` / `SRM_EXPEDITE_PO` | 采购建议、提交/审批采购申请、催单 |
| SAP | `SAP_CREATE_PR` / `SAP_RELEASE_PR` / `SAP_RELEASE_MO` | 创建/释放采购申请、释放生产订单 |
| BOM | `BOM_EXPLODE` / `BOM_SUBMIT_ECN` / `BOM_APPROVE_ECN` / `BOM_IMPLEMENT_ECN` | 展开、提交/审批/实施 ECN |
| INV | `INV_VERIFY_INPUT` / `INV_SUBMIT_REQUEST` / `INV_APPROVE_REQUEST` | 校验进项，提交/审核开票申请 |
| CRM | `CRM_ADVANCE_STAGE` / `CRM_ESCALATE_CASE` | 推进商机、升级工单 |
| DMS | `DMS_REPLENISH_SHORTAGE` / `DMS_PUSH_REPLENISH` | 按缺货生成补货单、按单号下发 |
| OA | `OA_START_WORKFLOW` / `OA_APPROVE_TASK` | 发起审批、审批待办。登录模式不执行 |

## 7. 集成契约

有 `X-Api-Key` 时优先 `GET/POST /api/open/ir/*`，`/actions` 出现可判定失败再回退专用口。没有 Key 时的登录业务口、仓号映射和仍必须带 Key 的系统，以 README「对接」为准。BMS 只读：`GET /api/open/cost/records?from&to`。

十二套同机端口（`data.sql`）：OMS 8081、TMS 8082、WMS 8083、BMS 8084、SAP 8085、OA 8086、SRM 8087、BOM 8088、INV 8089、IR 8090、CRM 8091、DMS 8092。各系统单独启动时的 `application.yml` 端口可以不同。
...
sequenceDiagram
    ...
    EXT-->>ACT: 执行结果
    ACT->>SNAP: mutateSnapshot 回写(改仓/挂起/加急/取消/分配/换承运商/追轨迹/调度)
...
- **成本节约口径**：`expectedSaving` / 回写 `actualSaving` 仍是快照运价差，尚未按 BMS 账单逐单对账；汇总运费已优先认 BMS。
