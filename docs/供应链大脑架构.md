...
- **策略沙盘 `SandboxEngine`**（详见 §4.2）：在不影响真实系统的前提下推演库存/需求/仓配策略，输出成本、服务水平、缺货、日序列与**成本-效率综合得分**。分为**人工沙盘**（业务人员自定义参数运行评估）与**系统自动沙盘推演**（系统在候选空间自动跑批、按成本与效率兼顾排名并推荐最优方案），支持 baseline 与场景对比，并可一键把推荐方案下发为跨系统协同动作。
- **成本分析 `CostService`**：按类型/仓库/承运商/趋势汇总，单均成本、目标 vs 实际、以及由成功动作 `expectedSaving` 累计的“节省额”。

### ⑤ 行动层（Action）— `com.ir.action`

把决策“落下去”，形成闭环。

- **`ActionService.createAndExecute`**：根据动作类型前缀路由到目标系统（`OMS_/WMS_/TMS_` → 对应系统；其余 → `SRM`），构造 `ActionCommand` 调用对应客户端 `execute`，执行成功后 `mutateSnapshot` 同步回写本地快照（如改仓、取消、分配、换承运商），状态 `PENDING→SUCCESS/FAILED`，支持 `retry` 与 `expectedSaving`。
- **`SRM` 特例**：因 SRM 未接入，`SRM_PURCHASE_SUGGEST` 不真正外调，仅生成“采购建议”。
- **闭环入口**：预警 `executeSuggested(alertId)` 与预测 `replenish/to-action` 均汇聚到 `ActionService`，把“发现问题/预测缺货 → 建议动作 → 跨系统执行 → 回写”串成一条自动化链路。
...
- **补货建议 `replenish`**：对每个库存项跑 AUTO 预测，`日均 = 预测总量/horizon`，`安全库存 = 日均 × serviceDays`，`建议补货 = 预测总量 + 安全库存 − 可用库存`，并推算 `stockoutDate`（缺货日）。可一键转 `SRM_PURCHASE_SUGGEST` 动作。

### 4.2 策略沙盘（`SandboxEngine`）：人工沙盘 + 系统自动推演

沙盘演练分为两种模式（`ct_scenario.mode`）：

- **人工沙盘（MANUAL）**：业务人员在界面自定义需求倍率、渠道倍率、分仓策略、承运结构、安全库存与**成本权重**，运行后得到该方案的成本、服务水平、时效、缺货与**相对基线的成本-效率综合得分**，并可与基线/其它场景对比、一键应用为协同动作。
- **系统自动沙盘推演（AUTO）**：系统自动遍历候选空间 **分仓策略{就近/最低成本/均衡/单仓} × 安全库存天数{2/3/5} × 承运结构{经济/均衡/优质}（共 36 个组合）**，逐一仿真后按成本-效率兼顾目标统一打分排名，**推荐最优方案**并落库（mode=AUTO），可直接把推荐方案下发为跨系统协同动作。

**成本-效率兼顾目标（`SandboxEngine.assignCostEfficiencyScores`）**：在候选集合内对成本、服务水平、平均时效做 min-max 归一化，
`损失 = costWeight×成本损失 + (1−costWeight)×效率损失`，其中 `效率损失 = 0.7×服务损失 + 0.3×时效损失`，`综合得分 = 100×(1−损失)`。
`costWeight`（成本权重，0~1）是“成本 vs 效率”的取舍旋钮：1=成本优先、0=效率优先、0.5=均衡；由用户在界面通过滑块设置。
...
- **输出**：总成本、按类型/仓库/承运商成本、服务水平、缺货量、平均时效、单位履约成本、成本-效率综合得分、日序列、SKU 汇总；`baseline` 场景可与其它场景做 `compare` 对比。
- **协同闭环**：`POST /api/sandbox/auto` 推演 → 推荐方案 `apply` → 生成 `OMS_REROUTE_WAREHOUSE` / `TMS_SWITCH_CARRIER` / `SRM_PURCHASE_SUGGEST` 等协同动作，实现“推演—决策—下发”的成本效率协同闭环。
...
| SRM | `SRM_PURCHASE_SUGGEST` | 采购建议（预留，仅生成建议不外调） |
...
**已具备（当前构造）**
- OMS/WMS/TMS/BMS 四系统的 MOCK 感知与统一快照、看板与全链路追踪、8 条内置预警、跨系统协同动作、需求预测与补货、成本与节省分析、闭环回写。
- **沙盘演练：人工沙盘 + 系统自动沙盘推演**，以可调 `costWeight` 的成本-效率兼顾目标对候选方案打分排名，推荐最优方案并一键下发协同动作。

**待完善（后续演进）**
- **打通真实 HTTP 接入**：将 `ct_system.mode` 从 MOCK 切到 HTTP，联调各系统真实端点；BMS `/api/open/cost/records` 待上线。
- **接入 SRM 采购闭环**：当前 `SRM_PURCHASE_SUGGEST` 仅生成建议，未来对接 SRM（PR/RFQ/PO）形成“预警→缺货→自动补货采购”的完整闭环。
- **事件驱动替代轮询**：由当前 5 分钟定时同步演进为消息/事件推送，降低时延。
- **更丰富的规则与算法**：引入更多异常规则与机器学习预测，提升 AUTO 选优与置信区间质量。
- **纳入更多生态系统**：视需要把 SAP/INV 等的成本、税务、库存信号接入大脑视图。
