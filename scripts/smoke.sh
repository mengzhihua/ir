...
json -X POST "$BASE/api/sandbox/capital" -H 'Content-Type: application/json' -d '{"workingCapital":100000000}' | jq -e '.code==0 and .data.verdict=="RELIABLE" and .data.reliable==true and .data.baseline.capitalVerdict=="RELIABLE" and .data.demand5x.capitalVerdict=="RELIABLE" and (.data.headroom|tonumber)>20 and (.data.optimizations|length)>0 and .data.recommended.name=="低安全短交期" and .data.recommended.safetyDays==1 and .data.recommended.replenishLeadDays==1' >/dev/null
json -X POST "$BASE/api/sandbox/capital/adopt" -H 'Content-Type: application/json' -d '{"workingCapital":100000000}' | jq -e '.code==0 and (.data.name|test("低安全短交期")) and .data.kind=="MANUAL" and .data.id!=null' >/dev/null
