...
json "$BASE/api/trace/page" | jq -e '.code==0 and (.data|length)>0' >/dev/null
...
json "$BASE/api/alert/page" | jq -e '.code==0 and (.data|length)>0' >/dev/null
...
json "$BASE/api/cost/page" | jq -e '.code==0 and (.data|length)>0' >/dev/null
...
json "$BASE/api/system/user" | jq -e '.code==0 and (.data|length)>=3' >/dev/null
json "$BASE/api/system/op-log/page" | jq -e '.code==0' >/dev/null
