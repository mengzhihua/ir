...
json "$BASE/api/sandbox/auto/latest" | jq -e '.code==0 and .data.recommended.id!=null and (.data.recommended.serviceLevel|tonumber)>=0.995 and (.data.recommended.stockoutUnits|tonumber)==0 and .data.rationale.rule=="SERVICE_FIRST_CASH_ROBUST" and (.data.scenarios|length)>=10' >/dev/null
json "$BASE/api/sandbox/auto/history" | jq -e '.code==0 and (.data|length)>=1' >/dev/null
