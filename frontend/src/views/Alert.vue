<template>
  <div class="page">
    <div class="page-title">
      <div>
        <h2>预警中心</h2>
        <p class="subtitle">统一处理订单、库存、运输和成本风险</p>
      </div>
      <el-button v-if="canWrite()" type="primary" @click="evaluate">立即评估</el-button>
    </div>
    <div class="stats">
      <div class="stat">
        <div class="label">开放预警</div>
        <div class="value">{{ stats.open || 0 }}</div>
      </div>
      <div class="stat">
        <div class="label">高等级</div>
        <div class="value danger">{{ stats.high || 0 }}</div>
      </div>
      <div class="stat">
      ...
      <div class="stat">
        ...
        <div class="value success">{{ stats.resolved || 0 }}</div>
      </div>
    </div>
