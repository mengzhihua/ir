...
const compare = [
  ...
  },
  {
    ...
    ours: 'TMS 按时效、费率或历史评分择优，按坐标把订单绑到空闲车，承运商可填自己的取号地址',
    next: '绑车换成真实地图底图'
  },
  {
...
const systems = [
  ...
  { kicker: '执行', name: 'TMS 运配', text: '运单、发车、在途和签收。调度台按坐标把待派订单绑到最近的空闲车。承运商可以填自己的取号地址。' },
...
const phases = [
  {
    ...
    title: '执行闭环和控制塔',
    lines: [
      '订单到出库、运单、签收，同一时段可以合单',
      '承运商评分、回货、报价说明里的距离和车型',
      '成本与时效的平衡、人工沙盘和自动沙盘'
    ]
  },
  {
    ...
    title: '单价、绑车和承运商取号地址',
    lines: [
      '计件单价可按仓库和货主覆盖，没配的节点仍用默认单价',
      '调度台按订单位置把待派订单绑到最近且装得下的空闲车',
      '承运商填了取号地址就走该地址，否则用全局地址。默认仍是本地模拟号'
    ]
  },
  {
    stage: '下一阶段',
    title: '店铺窗口、计价公式和地图底图',
    lines: [
      '合单窗口按店铺单独配置',
      '运费公式按距离和车型计价，说明里已经写上这两项',
      '绑车换成真实地图底图，当前只按坐标算距离'
    ]
  }
]
